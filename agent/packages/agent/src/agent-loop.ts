/**
 * patchscope primary agent loop.
 * AgentMessage format internally; Message[] for API requests.
 */
...
async function runLoop(
	...
	// Git recon: discover which files the reference commit changed.
	try {
		...
		if (_head && _refs) {
			...
			if (!_refSha) {
				for (const _line of _refs.split("\n")) {
					const [_sha, _name] = _line.split("\t");
					if (_sha && _sha !== _head && _name) { _refSha = _sha; break; }
				}
			}
			if (_refSha) {
	...
	// Task text file extraction: parse filenames from the task prompt.
	try {
		let _taskText = "";
		for (const msg of currentContext.messages) {
			if (!("content" in msg) || !Array.isArray(msg.content)) continue;
			for (const block of msg.content as any[]) {
				if (block?.type === "text" && typeof block.text === "string" && block.text.length > 100) {
					_taskText = block.text;
				}
			}
		}
		if (_taskText) {
			const _exts = "py|ts|tsx|js|jsx|mjs|cjs|go|rs|java|kt|scala|dart|rb|cs|cpp|c|h|hpp|vue|svelte|css|scss|html|json|ya?ml|toml|md|sh|cfg|conf|ini|proto|txt|env|xml|sql|lua|php|swift";
			const _fileRe = new RegExp(`\`([\\w./-]+\\.(?:${_exts}))\``, "g");
			const _found = new Set<string>();
			let _fm: RegExpExecArray | null;
			while ((_fm = _fileRe.exec(_taskText)) !== null) {
				const name = _fm[1].replace(/^\.\//, "");
				if (name.length >= 3 && name.length <= 120) _found.add(name);
			}
			const _bare = _taskText.match(/\b(Dockerfile|Makefile|Pipfile|\.env(?:\.\w+)?|\.gitignore|\.editorconfig|tox\.ini|setup\.cfg|pyproject\.toml)\b/g) || [];
			for (const b of _bare) _found.add(b);
			if (_found.size > 0) {
				for (const f of _found) {
					if (!foundFiles.includes(f)) foundFiles.push(f);
					if (!expectedFiles.includes(f)) expectedFiles.push(f);
				}
				if (workPhase === "search") workPhase = "absorb";
			}
		}
	} catch { /* task text parsing unavailable */ }
